...
async def show_register_form(request: Request, message: str = None, error: str = None):
    ...
    """
    return templates.TemplateResponse("register.html", {"request": request, "message": message, "error": error})


@app.post("/register", response_class=HTMLResponse, summary="사용자 등록 처리 (Form)")
async def handle_registration_form(request: Request, phone_number: str = Form(...), name: str = Form(...)):
    """
    ...
    try:
        ...
        print(f"폼을 통해 새로운 사용자 등록 시도: {user_data.name} ({user_data.phone_number})")
        ...
        # 유효성 검사 실패 시, 에러 메시지와 함께 폼을 다시 렌더링합니다.
        error_message = "입력값을 확인해주세요: " + ", ".join([err['msg'] for err in e.errors()])
        ...
        return templates.TemplateResponse("register.html", context, status_code=status.HTTP_422_UNPROCESSABLE_ENTITY)


# --- 기존 JSON API 엔드포인트 ---

@app.post("/users/", response_model=UserResponse, status_code=status.HTTP_201_CREATED, summary="신규 사용자 등록 (API)")
